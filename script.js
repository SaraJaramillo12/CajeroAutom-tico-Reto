...
function encontrarUsuario(documento, contraseña) {
    return usuarios.find(user => user.documento === documento && user.contraseña === contraseña);
}
...
function mostrarResumen() {
    ...
    billetes.forEach(billete => {
        output.innerHTML += `<p>Denominación: ${billete.denominacion}, Cantidad: ${billete.cantidad}</p>`;
...
function iniciarSesion() {
    ...
    if (!usuario) {
        ...
        let totalDinero = billetes.reduce((sum, billete) => sum + billete.denominacion * billete.cantidad, 0);
        if (totalDinero === 0) {
...
function cargarCajero() {
    billetes[0].cantidad += parseInt(document.getElementById('billetes100000').value || 0);
    billetes[1].cantidad += parseInt(document.getElementById('billetes50000').value || 0);
    billetes[2].cantidad += parseInt(document.getElementById('billetes20000').value || 0);
    billetes[3].cantidad += parseInt(document.getElementById('billetes10000').value || 0);
    billetes[4].cantidad += parseInt(document.getElementById('billetes5000').value || 0);
    mostrarResumen();
    reset();
}
